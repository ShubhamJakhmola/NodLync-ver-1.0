...
const ProjectForm = ({
  ...
  return (
    <div className="h-full flex flex-col border border-stroke/40 rounded-xl bg-surface/5 overflow-hidden">
      ...
      {/* Form */}
      <form className="p-6 md:p-8 space-y-5 flex-1 overflow-y-auto" onSubmit={submit}>
        {/* Name field */}
        <label className="block space-y-2">
          ...
          <input
            className={`w-full rounded-lg border bg-surface px-4 py-3 text-sm font-medium text-fg shadow-inner focus:outline-none focus:ring-2 focus:ring-primary/50 transition ${
        ...
        {/* Description field */}
        <label className="block space-y-2">
          <span className="text-[11px] font-black uppercase tracking-widest text-fg-muted">Description</span>
          <textarea
            className="w-full rounded-lg border border-stroke bg-surface px-4 py-3 text-sm text-fg shadow-inner focus:outline-none focus:ring-2 focus:ring-primary/50 transition resize-none leading-relaxed"
        ...
        {/* Status field */}
        <label className="block space-y-2">
          <span className="text-[11px] font-black uppercase tracking-widest text-fg-muted">Current Status</span>
          <select
            className="w-full rounded-lg border border-stroke bg-surface/50 px-4 py-3 text-sm font-medium text-fg-secondary focus:outline-none focus:ring-2 focus:ring-primary/50 transition cursor-pointer appearance-none"
            value={status}
        ...
        <div className="flex items-center gap-3 pt-4">
          <button
            type="submit"
            className="btn-primary text-xs font-black uppercase tracking-widest px-6 py-3 shadow-lg shadow-primary/25 hover:shadow-primary/40 active:scale-[0.98]"
            disabled={busy}
          >
            {busy
              ? "SAVING..."
          ...
          {mode === "edit" && onDelete && (
            <button
              type="button"
              className={`px-5 py-3 text-xs font-black uppercase tracking-widest rounded-lg border transition active:scale-[0.98] ${
                confirmDelete
